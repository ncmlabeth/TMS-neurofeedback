...
bigger_than_mean_fake_MEPS=((mean_MEP*2)-mean_MEP).*rand(number_nonrewards,1)+mean_MEP; %swap the rewards and nonrewards variables in UP and DOWN scripts
smaller_than_mean_fake_MEPS=(mean_MEP-(mean_MEP*2)).*rand(number_rewards,1)+mean_MEP;
...
for pos=1:length(reward_positions)
    control_MEP_vector(reward_positions(pos))=smaller_than_mean_fake_MEPS(b)  %specific to DN condition
    b=b+1
end
b=1;
for pos=1:length(no_reward_positions)
    control_MEP_vector(no_reward_positions(pos))=bigger_than_mean_fake_MEPS(b) % specific to DN condition
    b=b+1
end    
...
nChans = 6;  % number of EMG channels
...
minfixGap = 1; maxfixGap = 1;   % I set these small becasue the main gap will occur AFTER the TMS machines have armed and made their noises- to help avoid anticipation
fixgapDurationArray = (rand(nMEPs,1)*maxfixGap) + minfixGap;  % array of gap duration between trials
minWaitGap = 1; maxWaitGap = 3;
WaitgapDurationArray = (rand(nMEPs,1)*maxfixGap) + minfixGap;  % array of gap duration between trials
...
dataFilename = [direc filesep 'data' filesep subCode filesep SesNum filesep 'DN_b'  num2str(blockNumber) '-m' num2str(nMEPs) '-c' num2str(nChans) '.dat'];
...
s = serial('COM4');  % COM number is machine specific!
...
% open serial port
fopen(s);  
% fwrite(s,'1,1,0*'); % arm stimulator
% WaitSecs(1); 
fwrite(s,['1,2,' intensity_string '*']); % set power to required intensity
WaitSecs(1); 
...
rawRMSData = [];
dummyData=zeros(1000,nChans+1); %1000 samples is 500ms
dummyData(:,:)=0.005;
...
for ii = 1:nMEPs
    ...
    while exitTrial == false
        ...
        if trialState == 3 && triggerSent == 0  ...
            ...
            fwrite(s,'1,4,0*'); % trigger single pulse
            triggerSent = 1;
            mepCount = mepCount + 1;
        end
   ...
   %Caclulate the position of the feedback bar ( THIS IS ADAPTED FOR
   %CONTROL COND- ALWAYS AT THE MEAN -middle)
...
%           
        ...
        elseif trialState == 2 && vbl - fixStartTime > fixgapDurationArray(ii,1) && FDIrmsgood==1  && ch3rmsgood==1 && ch4rmsgood==1 && ch5rmsgood==1 ;
           fwrite(s,'1,6,0*'); % disarm stimulator
            fwrite(s,'2,6,0*'); % disarm stimulator 2
            WaitSecs(1);
            fwrite(s,'1,1,0*'); % arm stimulator
            fwrite(s,'2,1,0*'); % arm stimulator 2
            WaitSecs(WaitgapDurationArray(ii,1));
        ...
        elseif trialState  == 3 && vbl - emgStartTime > emgDuration
            ...
            feedbackData = uniqueEpochData(:,2:end); %first channel is timestamps
             save([direc filesep 'data' filesep subCode filesep SesNum filesep 'exp_DN' num2str(blockNumber) '-mep' (num2str(ii)) '.mat'], 'feedbackData','rawRMSData' )
            TMS=feedbackData(:,1); %first column of feedback data is TMS channel
