...
percentage_reward=0.66; % 50% of trials will have a reward
...
for pos=1:length(reward_positions)
    control_MEP_vector(reward_positions(pos))=smaller_than_mean_fake_MEPS(b) %specific to Down condition
    b=b+1
end
b=1;
for pos=1:length(no_reward_positions)
    control_MEP_vector(no_reward_positions(pos))=bigger_than_mean_fake_MEPS(b) %specific to Down condition
    b=b+1
end    
...
minGap = 4; maxGap = 5;   % set min and max gap times
...
% file handling
%% file handling
subDir = [direc filesep 'data' filesep subCode filesep 'EEG'];
if ~exist(subDir,'dir')
    mkdir(subDir);
end

dataFilename = [direc filesep 'data' filesep subCode filesep 'EEG' filesep 'DN_b'  num2str(blockNumber) '-m' num2str(nMEPs) '-c' num2str(nChans) '.dat'];
...
for ii = 1:nMEPs
    ...
    while exitTrial == false
        ...
        elseif trialState  == 3 && vbl - emgStartTime > emgDuration
            ...
            save([direc filesep 'data' filesep subCode filesep 'EEG' filesep 'exp_DN' num2str(blockNumber) '-mep' (num2str(ii)) '.mat'], 'feedbackData','rawRMSData' )
...
save([direc filesep 'data' filesep subCode filesep 'EEG' filesep 'DN_b' num2str(blockNumber) '-success' (num2str(nSuccess)) '.mat'], 'nSuccess', 'percentage_success')%save the num of successful trials
